...
class ResultPDF(FPDF):
    def draw(self, d, subs, logo):
        ...
        self.ln(10); self.set_font("Helvetica", 'I', 9); self.multi_cell(190, 5, '"Education is the most powerful weapon which you can use to change the world."', align='C')
...
class AwardListPDF(FPDF):
    def generate(self, data_list, title="AWARD LIST"):
        ...
        self.cell(15, 10, "Pos", 1, 0, 'C', True); self.cell(20, 10, "Roll", 1, 0, 'C', True); self.cell(25, 10, "Class", 1, 0, 'C', True); self.cell(75, 10, "Student Name", 1, 0, 'C', True); self.cell(25, 10, "Marks", 1, 0, 'C', True); self.cell(30, 10, "%", 1, 1, 'C', True)
        ...
        for r in data_list:
            ...
            self.cell(25, 8, r.get('Class', ''), 1, 0, 'C', True)
            self.cell(75, 8, f" {str(r['Name']).upper()}", 1, 0, 'L', True)
            self.cell(25, 8, str(r['Total']), 1, 0, 'C', True)
            self.cell(30, 8, f"{r['Perc']:.1f}%", 1, 1, 'C', True)
...
if not df.empty and "Class" in df.columns:
    ...
    if not fil.empty:
        # Dynamic calculation based on selected subjects
        fil['Total_Obtained'] = fil[sel].apply(pd.to_numeric, errors='coerce').fillna(0).sum(axis=1)
        # Dynamic denominator based on SUBJECTS total
        total_possible = sum([int(fil.iloc[0].get(f"Total_{s}", 50)) for s in sel])
    ...
    if fil.empty: st.warning("No data.")
        ...
        with c2:
            ...
            if st.button("Generate Current Class Award List"):
                aw_list = []
                total_pos = sum([int(fil.iloc[0].get(f"Total_{s}", 50)) for s in sel])
                for _, r in fil.iterrows():
                    obt = sum([int(r.get(s, 0)) for s in sel])
                    aw_list.append({"Roll No": r['Roll No'], "Name": r['Name'], "Total": obt, "Perc": (obt/total_pos*100) if total_pos>0 else 0, "DisplayPos": str(r['Position']) if 1 <= r['Position'] <= 5 else "---"})
                pdf_aw = AwardListPDF(); pdf_aw.generate(aw_list, f"AWARD LIST - {cl} ({sc})")
                st.download_button(f"AwardList_{cl}.pdf", bytes(pdf_aw.output()), f"AwardList_{cl}.pdf")
            
            st.divider()
            # --- NEW FINAL TOPPERS FUNCTION ---
            if st.button("🏆 Generate ALL CLASSES TOPPERS LIST"):
                ...
                for c_name in CLASSES:
                    ...
                    if not c_df.empty:
                        ...
                        c_df['Total_Obtained'] = c_df[sel].apply(pd.to_numeric, errors='coerce').fillna(0).sum(axis=1)
                        # Find Top 5
                        top5 = c_df.sort_values("Total_Obtained", ascending=False).head(5)
                        # Determine total marks for percentage
                        total_pos = sum([int(top5.iloc[0].get(f"Total_{s}", 50)) for s in sel])
                        for idx, r in enumerate(top5.to_dict('records')):
                            r['DisplayPos'] = str(idx + 1)
                            r['Total'] = r['Total_Obtained']
                            r['Perc'] = (r['Total'] / total_pos * 100) if total_pos > 0 else 0
                            all_toppers.append(r)
                
                pdf_all = AwardListPDF()
                pdf_all.generate(all_toppers, "GRAND TOPPERS LIST (TOP 5 FROM EACH CLASS)")
                st.download_button("Download_All_Toppers.pdf", bytes(pdf_all.output()), "Grand_Toppers_List.pdf")
